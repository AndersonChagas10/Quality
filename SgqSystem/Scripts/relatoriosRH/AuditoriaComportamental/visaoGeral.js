...
function montaCardsAcompanhamento(listaAuditoria, totalSemanas) {
    ...
    listaAuditoria.map(function (o, i) {
        ...
        if (o['C'] != "" && o['C'] != undefined) {
            totalConforme += parseInt(o['C']);
        }
        if (o['NC'] != "" && o['NC'] != undefined) {
...
function enviarFiltro(nivelVisao) {
    ...
    switch (nivelVisao) {
        ...
        case '3':
            ...
            }).done(function (listaAuditor) {
                listaGlobalAuditores = listaAuditor;
            }).fail(function (jqXHR, textStatus, msg) {
                console.log(msg);
                closeLoader();
                    ...
                    var colunas = [
                        ...
                        { title: "Total", mData: "total" },
                        {
                            title: "% Realizado", mData: null, mRender: function (acompanhamentoObj, type, full) {
                                ...
                                for (var i = 0; i < listaDeSemanas.length; i++) {
                                    ...
                                    if (acompanhamentoObj[listaDeSemanas[i].title] != undefined) {
                        ...
                        },
                        {
                            title: "% Seguro", mData: null, mRender: function (acompanhamentoObj, type, full) {
                                ...
                                if (acompanhamentoObj.C != undefined) {
                                    porcentagemTotal = acompanhamentoObj.C / acompanhamentoObj.total * 100;
                                } else {
                                    porcentagemTotal = 0;
                                }
                        ...
                        },
                        {
                            title: "% Inseguro", mData: null, mRender: function (acompanhamentoObj, type, full) {
                                ...
                                if (acompanhamentoObj.NC != undefined) {
                                    porcentagemTotal = acompanhamentoObj.NC / acompanhamentoObj.total * 100;
                                }else {
                                    porcentagemTotal = 0;
                                }
