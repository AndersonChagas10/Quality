    ...
    public class UserSgqController : BaseController
    {
        ...
        public ActionResult Perfil(string motivo = "")
        {
            ...
            var model = new UserViewModel()
            {
                ...
                Empresa = parCompanyXUserSgq != null ? (from comp in parCompanyXUserSgq select new EmpresaDTO { Role = comp.Role, Nome = comp.ParCompany.Name, Perfil = Perfis.Where(x => x.nCdPerfil.ToString() == comp.Role).FirstOrDefault().cNmPerfil }).ToList() : new List<EmpresaDTO>()
            };
