        ...
        public IHttpActionResult SalvarExcel([FromBody] List<JObject> dados, int formatoId)
        {
            ...
            try
            {
                ...
                foreach (var item in divjetoFinal)
                {
                    ...
                    {
                        mensagemErro += "Ocorreu um erro inesperado na linha: " + contador + "<br />";                        
                    }
                }
                //Replace("\r\n", Environment.NewLine)
                return ResponseMessage(Request.CreateResponse(HttpStatusCode.OK, String.Join("", mensagemErro) + "Processado com sucesso"));
                //return Ok(String.Join("", mensagemErro)  + "Processado com sucesso");
            }
            catch (Exception e)
            {
