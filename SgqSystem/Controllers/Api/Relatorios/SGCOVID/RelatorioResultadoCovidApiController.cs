    ...
    public class RelatorioResultadoCovidApiController : ApiController
    {
        ...
        public List<JObject> GetRelatorio([FromBody] DataCarrierFormularioNew form)
        {
            ...
            string sqlQuery = $@"
				...
				SELECT
	CAST(C2.CollectionDate AS DATE) AS __Data
   , DATEPART(YEAR, CAST(C2.CollectionDate AS DATE)) AS ""_Ano""
   ,DATEPART(MONTH, CAST(C2.CollectionDate AS DATE)) AS ""_Mês""
   ,(DAY(CAST(C2.CollectionDate AS DATE)) + (DATEPART(dw, DATEADD(MONTH, DATEDIFF(MONTH, 0, CAST(C2.CollectionDate AS DATE)), 0)) - 1) - 1) / 7 + 1 AS ""_ Semana Do Mês""
   ,DATEPART(DAY, CAST(C2.CollectionDate AS DATE)) AS ""_Dia""
...
--AND C2.CollectionDate >= '2020-05-01'
{whereUnidade}
{whereDate}
{whereCluster}
{whereStructure2}
{whereStructure3}
GROUP BY CAST(C2.CollectionDate AS DATE)
		,C2.ParFrequency_Id
