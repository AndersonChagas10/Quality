...
$(document).on('click', '.level01List .level01', function (e) {
    ...
    if (resultPeriod.attr('completereaudit')) {
    ...
    CreateHeader($('.level02List .painel'), getLevel1($(this).attr('id')).cabecalhosInclusos, getLevel1($(this).attr('id')).contadoresIncluidos);
...
$(document).on('click', '.level02List .level02', function (e) {
    CreateHeader($('.level03List .painel'), getLevel2($(this).parents('.level02Group').attr('level01id'), $(this).attr('id')).cabecalhosInclusos, getLevel2($(this).parents('.level02Group').attr('level01id'), $(this).attr('id')).listParCounterXLocal);
    showLevel03($(this));
});
...
function breadCrumb(level01, level02) {
    ...
    var button = "<button class='btn btn-primary disabled pull-right' style='margin: -8px -15px; padding-bottom: 8px;'>"+
                    "<span class='atualDate'>"+dateTimeWithMinutes()+"</span> / "+
                    "<span class='shift'>"+$('#shift option:selected').text()+"</span>"+
                 "</button>";
...
$(document).on('click', '#btnSendCorrectiveAction', function (e) {
    ...
    var correctiveAction = $('.correctiveAction[idcorrectiveaction=' + level02Result.attr('idcorrectiveaction') + ']');
    if (correctiveActionModal.attr('idcorrectiveaction')) {
        correctiveAction = $('.correctiveAction[idcorrectiveaction=' + correctiveActionModal.attr('idcorrectiveaction') + ']');
    }
    if (correctiveAction.length) {
        correctiveAction.attr('descriptionFailure', $('#correctiveActionModal #DescriptionFailure').val());
        correctiveAction.attr('immediateCorrectiveAction', $('#correctiveActionModal #ImmediateCorrectiveAction').val());
        correctiveAction.attr('productDisposition', $('#correctiveActionModal #ProductDisposition').val());
        correctiveAction.attr('preventativeMeasure', $('#correctiveActionModal #PreventativeMeasure').val());
        correctiveAction.attr('slaugtherSignature', $('.SlaugtherSignature').attr('userid'));
        correctiveAction.attr('techinicalSignature', $('.TechinicalSignature').attr('userid'));
        ...
        $('.level02Result[idcorrectiveaction=' + correctiveActionModal.attr('idcorrectiveaction') + ']').attr('sync', 'false').attr('datetime', dateTimeFormat);
        ...
        correctiveAction.attr('descriptionFailure', descriptionFailure);
        ...
        correctiveAction.attr('immediateCorrectiveAction', immediateCorrectiveAction);

        // var immediateCorrectiveActionSpan = $(document.createElement('div'));
        ...
        // var productDisposition = $(document.createElement('div'));
        // productDisposition.addClass('productDisposition');
        // productDisposition.text(immediateCorrectiveAction);

        correctiveAction.attr('preventativeMeasure', preventativeMeasure);

        // var preventativeMeasureSpan = $(document.createElement('div'));
        // preventativeMeasureSpan.addClass('preventativeMeasure');
        // preventativeMeasureSpan.text(preventativeMeasure);

        // appendDevice(descriptionSpan, correctiveAction);
        // appendDevice(immediateCorrectiveActionSpan, correctiveAction);
        // appendDevice(productDisposition, correctiveAction);
        // appendDevice(preventativeMeasureSpan, correctiveAction);

        var idCorrectiveAction = $('.correctiveAction').length + 1;
        correctiveAction.attr('idCorrectiveAction', idCorrectiveAction);

        correctiveAction.attr('DateTimeSlaughter', $('.SlaugtherSignature').attr('datetime'));
        correctiveAction.attr('DateTimeTechinical', $('.TechinicalSignature').attr('datetime'));
        correctiveAction.attr('AuditStartTime', $('#AuditInformation').children('span#starttime').text());
        correctiveAction.attr('DateCorrectiveAction', $('#CorrectiveActionTaken').children('span#datetime').text());
        level02Result.attr('idCorrectiveAction', idCorrectiveAction).attr('sync', 'false').attr('datetime', dateTimeFormat());
    ...
    level02ResultsUpdate.attr('correctiveactioncomplete', 'correctiveActionComplete').removeAttr('havecorrectiveaction');
