...
namespace Conformity.Infra.Data.Core
{
    ...
    public class RepositoryBase<T> : IDisposable, IRepository<T> where T : BaseModel
    {
        ...
        /// </summary>
        /// <param name="Db"></param>
        public RepositoryBase(EntityContext dbContext)
        {
        ...
        {
            using (var ts = _dbContext.Database.BeginTransaction(IsolationLevel.ReadUncommitted))
            {
                if (obj.GetType().GetProperty("Id") != null)
                {
                    var id = (int)obj.GetType().GetProperty("Id").GetValue(obj, null);
        ...
        {
           return _dbContext.Database.ExecuteSqlCommand(v);
        }

        public List<T> ExecuteSqlQuery(string v)
        {
            return _dbContext.Database.SqlQuery<T>(v).ToList();
        }
