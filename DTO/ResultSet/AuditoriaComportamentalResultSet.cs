...
namespace DTO.ResultSet
{
    public class AuditoriaComportamentalResultSet
    {
        ...
        public string GetVisaoGeral(DataCarrierFormularioNew form, string userUnits)
        {
            ...
            DECLARE @DATAINICIAL DATETIME = '2020-10-19'
        ...
        public string GetVisaoUnidade(DataCarrierFormularioNew form, string userUnits)
        {
            ...
            SELECT 
	            ...
	            ,CASE
		            WHEN ISNULL(R3.ValueText, '') IN ('undefined', 'null') THEN ''
		            ELSE ISNULL(R3.ValueText, '')
	            END as 'ValorDescricaoTarefa', 
	           Iif(Charindex('""Auditor""', HF.HeaderFieldList1) > 0, 
               Substring(HF.HeaderFieldList1,
               Charindex('""Auditor""', HF.HeaderFieldList1)
               + Len('""Auditor"":""'), Charindex('""',
               Substring(HF.HeaderFieldList1,
               Charindex('""Auditor""', HF.HeaderFieldList1)
               + Len('""Auditor"":""'),
               Len(HF.HeaderFieldList1))) - 1), '') AS [Auditor Cabecalho],
               Iif(Charindex('""Avaliação da Atividade""', HF.HeaderFieldList1) > 0, 
               Substring(HF.HeaderFieldList1,
               Charindex('""Avaliação da Atividade""',
               HF.HeaderFieldList1) + Len('""Avaliação da Atividade"":""'),
               Charindex('""', Substring(HF.HeaderFieldList1,
               Charindex('""Avaliação da Atividade""',HF.HeaderFieldList1)
               + Len('""Avaliação da Atividade"":""'), Len(
               HF.HeaderFieldList1))) - 1), '') AS [Avaliação da Atividade],
               Iif(Charindex('pessoas observadas""', HF.HeaderFieldList1) > 0, 
               Substring(HF.HeaderFieldList1, Charindex('pessoas observadas""',HF.HeaderFieldList1) + Len('pessoas observadas"":""'),
               Charindex('""', Substring(HF.HeaderFieldList1,Charindex('pessoas observadas""',
               HF.HeaderFieldList1)+ Len( 'pessoas observadas"":""'), Len(HF.HeaderFieldList1)))- 1), '') AS[pessoas observadas],
               Iif(Charindex('""Pessoa avaliada""', HF.HeaderFieldList1) > 0, 
               Substring(HF.HeaderFieldList1, Charindex('""Pessoa avaliada""', HF.HeaderFieldList1) + Len('""Pessoa avaliada"":""'),
               Charindex('""', Substring(HF.HeaderFieldList1,Charindex('""Pessoa avaliada""',HF.HeaderFieldList1) + Len('""Pessoa avaliada"":""'),
               Len(HF.HeaderFieldList1))) - 1), '') AS [Pessoa avaliada],
               Iif(Charindex('""Tipo de Tarefa Realizada""', HF.HeaderFieldList1) > 0, 
               Substring(HF.HeaderFieldList1,Charindex('""Tipo de Tarefa Realizada""',HF.HeaderFieldList1)
               + Len('""Tipo de Tarefa Realizada"":""'),
               Charindex('""', Substring(HF.HeaderFieldList1,
             ...
             Iif(Charindex('""Data da Auditoria""', HF.HeaderFieldList1) > 0, 
                           Substring(HF.HeaderFieldList1,
                           Charindex('""Data da Auditoria""', HF.HeaderFieldList1)
                           + Len('""Data da Auditoria"":""'), Charindex('""',
                           Substring(HF.HeaderFieldList1,
                           Charindex('""Data da Auditoria""', HF.HeaderFieldList1)
                           + Len('""Data da Auditoria"":""'),
                           Len(HF.HeaderFieldList1))) - 1), '') AS[Data Auditoria]
            INTO #Coletas
            FROM #CollectionLevel2 C2 (NOLOCK)
            INNER JOIN ParCompany UN with (NOLOCK)
	            ON UN.Id = C2.UnitId
...
Begin
            ...
            SET @SQLStr = 'Select indicador,secao, grupoempresa,Data, Unidade,collectionl2_id,monitoramento, tarefa, conforme, valordescricaotarefa, cargo, CentroCusto,regional, IIF([Data da Auditoria] <> '''', CONCAT(SUBSTRING([Data da Auditoria], 9,2),''/'',SUBSTRING([Data da Auditoria], 6,2 ),''/'', SUBSTRING([Data da Auditoria], 1,4 )),'''') as [Data da Auditoria] , [Auditor Cabecalho],SUM(CAST(IIF(LEN([pessoas observadas]) > 0, [pessoas observadas], 0) as decimal)) as [pessoas observadas],[Atividade],[Medidas / Ação tomadas para correção ou incentivo do comportamento],[Hora da Auditoria], [Avaliação da Atividade], [Pessoa avaliada],[Tipo de Tarefa Realizada],'
            ...
            + 'IIF(CHARINDEX(''""Data da Auditoria""'', HeaderFieldListL1) > 0, SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Data da Auditoria""'', HeaderFieldListL1)+LEN(''""Data da Auditoria"":""''),CHARINDEX(''""'',SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Data da Auditoria""'', HeaderFieldListL1)+LEN(''""Data da Auditoria"":""''),LEN(HeaderFieldListL1)))-1), '''') as [Data da Auditoria],'
            ...
            + 'IIF(CHARINDEX(''""Atividade""'', HeaderFieldListL1) > 0, SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Atividade""'', HeaderFieldListL1)+LEN(''""Atividade"":""''),CHARINDEX(''""'',SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Atividade""'', HeaderFieldListL1)+LEN(''""Atividade"":""''),LEN(HeaderFieldListL1)))-1), '''') as [Atividade],'
            + 'IIF(CHARINDEX(''""Hora da Auditoria""'', HeaderFieldListL1) > 0, SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Hora da Auditoria""'', HeaderFieldListL1)+LEN(''""Hora da Auditoria"":""''),CHARINDEX(''""'',SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Hora da Auditoria""'', HeaderFieldListL1)+LEN(''""Hora da Auditoria"":""''),LEN(HeaderFieldListL1)))-1), '''') as [Hora da Auditoria],'
            + 'IIF(CHARINDEX(''""Medidas / Ação tomadas para correção ou incentivo do comportamento""'', HeaderFieldListL1) > 0, SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Medidas / Ação tomadas para correção ou incentivo do comportamento""'', HeaderFieldListL1)+LEN(''""Medidas / Ação tomadas para correção ou incentivo do comportamento"":""''),CHARINDEX(''""'',SUBSTRING(HeaderFieldListL1, CHARINDEX(''""Medidas / Ação tomadas para correção ou incentivo do comportamento""'', HeaderFieldListL1)+LEN(''""Medidas / Ação tomadas para correção ou incentivo do comportamento"":""''),LEN(HeaderFieldListL1)))-1), '''') as [Medidas / Ação tomadas para correção ou incentivo do comportamento],'
            ...
            + ' group by Indicador,	Data, collectionl2_id,[Avaliação da Atividade],[Pessoa avaliada],[Tipo de Tarefa Realizada],secao, grupoempresa, monitoramento, tarefa, conforme, valordescricaotarefa, cargo,centroCusto, regional,[Auditor Cabecalho],[Data da Auditoria],[Atividade],[Medidas / Ação tomadas para correção ou incentivo do comportamento],[Hora da Auditoria],Unidade'
            + ' ORDER BY 1'
