	 ...
	 --------------------------------
			...
			WHEN CUBO2.Equacao = 'QtdeTLNC > 0 ? 0 : 100' THEN
				CASE 
					WHEN MIN(RESPOSTA) = 0 THEN 0 --ESTOREI O LIMITE
					WHEN SUM(RESPOSTA) = COUNT(DISTINCT(CUBO2.PARLEVEL3_ID)) THEN 100 --TIREI NOTA MÁXIMA
					ELSE 100 -- NEM ESTOUREI, NEM ZEREI
				END
...
";
        ...
        else if (form.ShowModeloGrafico_Id[0] == 3) //SEMANA
        {
            query += $@"-- SEMANA
						...
						FROM #CUBO1 C
						GROUP BY 
						CAST(YEAR(C.DataTruncada) AS VARCHAR) + '-' + CASE WHEN LEN(CAST(DATEPART(WEEK, C.DataTruncada) AS VARCHAR)) = 1 THEN '0' + CAST(DATEPART(WEEK, C.DataTruncada) AS VARCHAR) ELSE CAST(DATEPART(WEEK, C.DataTruncada) AS VARCHAR) END
						ORDER BY 1 ";
        }
