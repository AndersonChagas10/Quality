...
namespace SGQDBContext
{
    public partial class ParLevel1
    {
        ...
        private string quebraProcesso { get; set; }

        public ParLevel1(SqlConnection _db, string _quebraProcesso)
        {
            db = _db;
            quebraProcesso = _quebraProcesso;
        }
        ...
        public IEnumerable<ParLevel1> getParLevel1ParCriticalLevelList(int ParCompany_Id, string Level1ListId, DateTime dateCollection)
        {
            ...
            string sql = $@"SELECT
                        	*
                        FROM (SELECT
                        		CAST(C.Id AS VARCHAR) + '{ quebraProcesso }' + CAST(P1.Id AS VARCHAR) AS Id
...
HAVING SUM(VolumeAlerta) IS NOT NULL ";
    ...
    public partial class ParLevel2
    {
        ...
        public bool DisparaCorrectiveAction { get; set; }
        ...
        public ParLevel2(SqlConnection _db, string _quebraProcesso)
        {
            db = _db;
            quebraProcesso = _quebraProcesso;
        }
        ...
        public IEnumerable<ParLevel2> getLevel2ByIdLevel1(SGQDBContext.ParLevel1 parLevel1, DateTime dateCollection, int ParCompany_Id)
        {
            ...
            {
                string sql = "   SELECT '" + parLevel1.ParCluster_Id + quebraProcesso + @"' + CAST(PL2.Id AS VARCHAR)  AS Id, PL2.Id as ParLevel2_Id, PL2.Name AS Name, PL2.HasSampleTotal, PL2.HasTakePhoto, PL2.IsEmptyLevel3, AL.ParNotConformityRule_id, AL.Value, AL.CorrectiveAction as DisparaCorrectiveAction, AL.IsReaudit, PL2.ParFrequency_id, D.name as Departamento " +
                             ...
                             "\n GROUP BY PL2.Id, PL2.Name, PL2.HasSampleTotal, PL2.IsEmptyLevel3, AL.ParNotConformityRule_Id, AL.IsReaudit, AL.Value, AL.CorrectiveAction, PL2.ParFrequency_id, PL2.HasTakePhoto, PL2.ParFrequency_id, D.name              ";
            ...
            else
            {
                ...
                string sql = "\n SELECT '" + parLevel1.ParCluster_Id + quebraProcesso + @"' + CAST(PL2.Id AS VARCHAR)  AS Id, PL2.Id as ParLevel2_Id, PL2.Name AS Name, PL2.HasSampleTotal, PL2.HasTakePhoto, PL2.IsEmptyLevel3, AL.ParNotConformityRule_id, AL.Value, AL.CorrectiveAction as DisparaCorrectiveAction, AL.IsReaudit,PL2.ParFrequency_id, D.name as Departamento  " +
                         ...
                         "\n  EXISTS " +
                         "\n ( " +
                         "\n select number as a  from ParEvaluation (nolock)  where IsActive = 1 and ParLevel2_id = PL2.Id and (ParCompany_Id = " + ParCompany_Id + " or ParCompany_Id is Null) and ParLevel1_Id = " + parLevel1.ParLevel1_Id + " and ParCluster_Id = " + parLevel1.ParCluster_Id + " " +
                         "\n ) " +
                         "\n AND " +
                         "\n  EXISTS " +
                         "\n ( " +
                         "\n select number as a  from ParSample  (nolock) where IsActive = 1 and ParLevel2_id = PL2.Id and (ParCompany_Id = " + ParCompany_Id + " or ParCompany_Id is Null) and ParLevel1_Id = " + parLevel1.ParLevel1_Id + " and ParCluster_Id = " + parLevel1.ParCluster_Id + " " +
                         "\n ) " +
                         "\n GROUP BY PL2.Id, PL2.Name, PL2.HasSampleTotal, PL2.IsEmptyLevel3, AL.ParNotConformityRule_Id, AL.IsReaudit, AL.Value, AL.CorrectiveAction, PL2.ParFrequency_id, PL2.HasTakePhoto , PL2.ParFrequency_id, D.name                 " +
                         "\n ";
    ...
    public partial class ParLevel2Evaluate
    {
        ...
        public ParLevel2Evaluate(SqlConnection _db, string _quebraProcesso)
        {
            db = _db;
            quebraProcesso = _quebraProcesso;
        }
        ...
        public IEnumerable<ParLevel2Evaluate> getEvaluate(ParLevel1 ParLevel1, int? ParCompany_Id, DateTime DateCollection, int Shift_Id)
        {
            ...
            if (ParLevel1.hashKey == 2 && ParCompany_Id != null)
            {
                ...
                string sql = $@"SELECT CONCAT('{ ParLevel1.ParCluster_Id + quebraProcesso }', CAST(PL2.Id AS VARCHAR)) AS Id,
    ...
    public partial class ParLevel2Sample
    {
        ...
        public ParLevel2Sample(SqlConnection _db, string _quebraProcesso)
        {
            db = _db;
            quebraProcesso = _quebraProcesso;
        }
    ...
    public partial class ParLevel3
    {
        ...
        private SqlConnection db { get; set; }
        public ParLevel3() { }
        public ParLevel3(SqlConnection _db, string _quebraProcesso)
        {
            db = _db;
            quebraProcesso = _quebraProcesso;
        }
        ...
        public IEnumerable<ParLevel3> getLevel3ByLevel2(SGQDBContext.ParLevel1 ParLevel1, SGQDBContext.ParLevel2 ParLevel2, int ParCompany_Id, DateTime DateCollect)
        {
            ...
            getFrequencyDate(ParLevel2.ParFrequency_Id, DateCollect, ref dataInicio, ref dataFim);
           ...
           "\n  , L3V.DynamicValue ";
            ...
            sql += "\n  ) TOTAL  ORDER BY TOTAL.ParCompany_Id2 desc, ISNULL(TOTAL.ParLevel3Group_Name,'ZZZ') ASC, 2 ASC,  15  ASC , 16  ASC";
    ...
    public partial class ParFieldType
    {
        ...
        private SqlConnection db { get; set; }
        public ParFieldType() { }
        public ParFieldType(SqlConnection _db, string _ConnectionString)
        {
            db = _db;
    ...
    public partial class Generico
    {
        ...
        private SqlConnection db { get; set; }
        public Generico() { }
        public Generico(SqlConnection _db, string _ConnectionString)
        {
            db = _db;
            ConnectionString = _ConnectionString;
        }
    ...
    public partial class ParCounter
    {
        ...
        private SqlConnection db { get; set; }
        public ParCounter() { }
        public ParCounter(SqlConnection _db, string _quebraProcesso)
        {
            db = _db;
            quebraProcesso = _quebraProcesso;
        }
        ...
        public IEnumerable<ParCounter> GetParLevelXParCounterList(ParLevel1 ParLevel1, ParLevel2 ParLevel2, int Level)
        {
            try
            {
                ...
                if (ParLevel2 != null)
                {
                    lista = ParLevel2.Id.Replace(quebraProcesso, "|").Split('|');
                ...
                if (ParLevel1_Id > 0 || ParLevel2_Id > 0)
                {
                    ...
                    if (ParLevel1_Id > 0)
                    {
                        ...
                        sql = "SELECT Distinct PO.level, PC.Name as Counter, PO.Name as Local, '" + ParCluster_IdL1 + quebraProcesso + "' + CAST(PL.ParLevel1_Id AS VARCHAR) AS indicador FROM ParCounterXLocal PL (nolock)  " +
                    ...
                    else if (ParLevel2_Id > 0)
                    {
                        ...
                        sql = "SELECT Distinct PO.level, PC.Name as Counter, PO.Name as Local, '" + ParCluster_IdL2 + quebraProcesso + "' + CAST(PL.ParLevel2_Id AS VARCHAR) AS indicador FROM ParCounterXLocal PL (nolock)  " +
                              "LEFT JOIN ParCounter PC (nolock)  ON PL.ParCounter_Id = PC.Id " +
                              "LEFT JOIN ParLocal PO (nolock)  ON PO.Id = PL.ParLocal_Id " +
    ...
    public partial class NotConformityRule
    {
        ...
        public NotConformityRule getParNCRule(int NCR_Id, int P2_Id, string P1_Id)
        {
            try
            {
                ...
                const string quebraProcesso = "98789";

                var pL1 = P1_Id.IndexOf(quebraProcesso) > 0 ? P1_Id.Replace(quebraProcesso, "|").Split('|')[1] : P1_Id;
