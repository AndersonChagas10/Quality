...
namespace Dominio.Services
{
    ...
    public class ParamsDomain : IParamsDomain
    {
        ...
        public ParLevel1DTO GetLevel1(int idParLevel1)
        {
            ...
            var listL3L2L1 = parlevel1.ParLevel3Level2Level1.ToList();
            parlevel1Dto.listParLevel3Level2Level1Dto = Mapper.Map<List<ParLevel3Level2Level1DTO>>(listL3L2L1.Where(r => r.Active == true).ToList());/*Level 2 e 3 vinculados*/
        ...
        public bool RemVinculoL1L2(int idLevel1, int idLevel2)
        {
            ...
            using (var db = new SgqDbDevEntities())
            {
                ...
                {
                    if (result1?.Count() > 0)
                        sql1 = "DELETE ParLevel3Level2Level1 SET Active = 0 WHERE ParLevel1_Id = " + idLevel1 + " AND ParLevel3Level2_Id IN (select id from ParLevel3Level2 where parlevel2_id = " + idLevel2 + ")";
                    if (result2?.Count() > 0)
                        sql2 = "DELETE ParLevel2Level1 SET IsActive = 0 WHERE ParLevel1_Id = " + idLevel1 + " AND ParLevel2_Id = " + idLevel2;
                }
                else
                {
                    return false;
