...
namespace Dominio.AcaoRH.Email
{
    public class EmailAcaoVencidaNotificados : IEmail
    {
        ...
        public void MontarBody(Acao acao)
        {
	        ...
	        Seguindo o parâmetro da regra de Plano de Ação cadastrada no sistema SG-SEMST, informamos que a Ação descrita a seguir, em que você foi notificado, venceu em {acao.DataConclusao:""dd/MM/yyyy""}.
            Favor entrar em contato com o responsável { acao.UserResponsavel.Name } para atualizar o status da Ação no sistema.           
        ...
        {
            this.Subject = $@"Notificação de ação vencia no SG-SESMT – Desvio no indicador {acao.ParLevel1.Name}";
        }
