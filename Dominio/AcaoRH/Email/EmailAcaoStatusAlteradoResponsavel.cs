...
{
    class EmailAcaoStatusAlteradoResponsavel : IEmail
    {
        public EmailAcaoStatusAlteradoResponsavel(Acao acao)
        {
            MontarSybject(acao);
            MontarBody(acao);
            MontarTo(acao);
        ...
        public void MontarBody(Acao acao)
        {
            ...
            Olá!
            <br><br>
	        Seguindo o parâmetro da regra de Plano de Ação cadastrada no sistema SG-SEMST, informamos que a Ação descrita a seguir, de sua responsabilidade pela execução com prazo para conclusão em {(acao.DataConclusao != null ? acao.DataConclusao?.ToString("dd/MM/yyyy") : "")}, teve seu status alterado.
            <br><br>
